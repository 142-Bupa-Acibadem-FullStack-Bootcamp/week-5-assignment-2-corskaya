...
namespace Northwind.Bll
{
    public class UserManager : BllBase<User, DtoUser>, IUserService
    {
        ...
        public IResponse<DtoRegisterUser> Register(DtoRegisterUser registerUser)
        {
            var user = userRepository.Register(ObjectMapper.Mapper.Map<User>(registerUser));

            if (user == null)
            {
                user.Password = EncryptionManager.MD5Hash(registerUser.Password);
